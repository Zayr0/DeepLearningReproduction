...
from ms_model_estimation.training.networks.convolutional.ResNet101 import ResNet101
from ms_model_estimation.training.networks.model_layer.OpenSimTreeLayer import OpenSimTreeLayer
from ms_model_estimation.training.networks.model_layer.OpenSimTransitionLayer import OpenSimTransitionLayer
...
class OpenSimModel(nn.Module):
    ...
    def __init__(
        ...
        self.conv_resnet101_opensim = []
        self.bn_resnet101_opensim = []
        ...
        for i in range(1, self.cfg.MODEL.CONV_LAYER + 1):
            if i == self.cfg.MODEL.CONV_LAYER:
                self.conv_resnet101_opensim.append(
                    nn.Conv2d(self.cfg.MODEL.CONV_CHANNEL_SIZE[i - 1], self.opensimTransitionLayer.outputSize,
                              (1, 1)))
                if self.cfg.LAYER.OS_TRANSIT.BN:
                    self.bn_resnet101_opensim.append(torch.nn.BatchNorm2d(self.opensimTransitionLayer.outputSize))
            else:
                self.conv_resnet101_opensim.append(
                    nn.Conv2d(self.cfg.MODEL.CONV_CHANNEL_SIZE[i - 1], self.cfg.MODEL.CONV_CHANNEL_SIZE[i],
                              (1, 1)))
                self.bn_resnet101_opensim.append(torch.nn.BatchNorm2d(self.cfg.MODEL.CONV_CHANNEL_SIZE[i]))

        self.conv_resnet101_opensim = torch.nn.ModuleList(self.conv_resnet101_opensim)
        self.bn_resnet101_opensim = torch.nn.ModuleList(self.bn_resnet101_opensim)

    def forward(self, x, otherBodyPrediction=None):

        x = self.resnet101Layer(x)

        for i in range(1, self.cfg.MODEL.CONV_LAYER + 1):
            x = self.conv_resnet101_opensim[i - 1](x)
            if i == self.cfg.MODEL.CONV_LAYER:
                if self.cfg.LAYER.OS_TRANSIT.BN:
                    x = self.bn_resnet101_opensim[i - 1](x)
                continue
            else:
                x = self.bn_resnet101_opensim[i - 1](x)
                x = self.activation_resnet101_opensim(x)
